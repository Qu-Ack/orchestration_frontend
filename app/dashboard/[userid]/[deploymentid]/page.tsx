...
export default function DeploymentPage({
  ...
  async function getDeploymentStatus() {
    try {
      const depid = (await params).deploymentid;
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/active/${depid}`,
        {
          headers: {
            Authorization: localStorage.getItem("session")!,
          },
  ...
  useEffect(() => {
    if (deploymentid) {
      fetchContainerStats();
      const statsInterval = setInterval(fetchContainerStats, 120 * 1000);
  ...
  useEffect(() => {
    if (showLogs && deploymentid) {
      fetchLogs();
      const logsInterval = setInterval(fetchLogs, 30 * 1000);
  ...
  return (
    ...
    ${deploying ? "opacity-20" : "opacity-100"}
  `}
                >
                  {deploying ? deploymentState?.message : "Restart Deployment"}
                </button>
                <a
